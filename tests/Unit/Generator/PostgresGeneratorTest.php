...
use Rgalstyan\SymfonyAggregatedQueries\Generator\PostgresGenerator;
...
final class PostgresGeneratorTest extends TestCase
{
    public function testGeneratesSqlForJsonObjectAndWhere(): void
    {
        ...
        $generator = new PostgresGenerator($entityManager);
        ...
        $expectedSql = "SELECT e.*, (CASE WHEN rel_profile.id IS NULL THEN NULL ELSE json_build_object('id', rel_profile.id, 'name', rel_profile.name) END) AS profile FROM partners e LEFT JOIN profiles rel_profile ON rel_profile.id = e.profile_id WHERE e.status = ? ORDER BY e.created_at DESC LIMIT 50 OFFSET 10";
