    ...
    public function generate(
        ...
        foreach ($relations as $relation) {
            ...
            if ($relation->method === 'json_object') {
                $json = sprintf('json_build_object(%s)', $this->buildJsonObjectPairs($relationAlias, $relation->columns));
                $selects[] = sprintf(
            ...
            if ($relation->method === 'json_array') {
                $json = sprintf('json_build_object(%s)', $this->buildJsonObjectPairs('sub', $relation->columns));
                $subquery = sprintf(
                    '(SELECT COALESCE(json_agg(%s), \'[]\'::json) FROM %s sub WHERE sub.%s = %s.%s) AS %s',
                    $json,
                    $targetTable,
                    $relation->relatedColumn,
                    $baseAlias,
